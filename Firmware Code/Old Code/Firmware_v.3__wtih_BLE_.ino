...
/* sendSignal() takes in a float value (txValue), converts it to a string 
 * and sends that string to the app.
 */
void sendSignal(txFloat){
    char txString[8];
    dtostrf(txFloat, 1, 2, txString); // convert float value to string
    pcharacteristic->setValue(txString); //set value to be sent
    pCharacteristic->notify(); // Send the txString to the app
    Serial.print("*** Sent Value: "); //for debugging
